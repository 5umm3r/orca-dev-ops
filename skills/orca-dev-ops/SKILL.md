...
| Control the child | master only | `orca orchestration check --wait/reply`, `worker-show`, `orca terminal read/send` as fallback; never hand approval or follow-ups to the user |
...
3. Start the child with the full plan, the chosen model, and the effort mapped below. Name `cc-<topic>` for Claude and `cx-<topic>` for Codex. `--agent` cannot pass CLI flags, so create the worktree and the agent terminal, wait until the agent is ready, then bind it as a dispatched worker instead of sending the prompt directly:
   ```sh
   orca orchestration run-create --objective "<text>" --json   # or reuse `run-current` if a Run is already bound
   ...
   ```
   `worker-start --terminal` reuses the terminal, injects Orca's worker preamble plus the spec as input, and returns a `dispatchId`; it replaces the old `orca terminal send` of the prompt.
   ...
   **Required for every Codex child launch or restart:** explicitly pass `-a never` (no approval prompts) and `-s workspace-write` (writes limited to the worktree), plus `--add-dir <worktree git dir>` and `-c sandbox_workspace_write.network_access=true` so git and package installs keep working. Never use `--dangerously-bypass-approvals-and-sandbox`. Do not rely on the parent session's permissions or local config defaults, and do not use bare `--agent codex` for this workflow because it cannot pass these required flags. Preserve the chosen model and effort, and continue to enforce the Orca worktree rules through the task prompt. Note: the Codex child bound with `--terminal` is not verified yet; the `--agent codex` path was verified earlier, but it cannot pass the required flags above, so still launch Codex children through `--terminal` and treat the dispatch binding as unconfirmed.
   `--model` / `--effort` cannot be combined with `worker-start --terminal`; the model and effort go on the terminal command as above.
   Write the prompt file to the scratchpad: "<approved plan>. Allowed files: <files>. Done when: <conditions that make the task done, including which verification must pass>. Verification: <commands>. Follow the Orca worktree rules: implement now, leave changes uncommitted. When a step does not need the master's input, keep going; stop and ask through `orca orchestration ask` only when you cannot continue without the master, or before anything destructive (deleting files outside this worktree, `git reset --hard`/`git clean -fd`, large dependency changes, database or external-service operations). For multi-step tasks, keep `TASKS.md` at the worktree root updated (done / in progress / not started / found issues) and never commit it. Report once with `orca orchestration send --type worker_done --outcome succeeded|failed` whose body is the five-line report: result / changed files / verification (commands run + last lines of output) / blocked on master / found." Use `--setup skip` on `orca worktree create` for documentation-only tasks that need no `node_modules`.
   "Done when" is required; do not send a prompt without it. Before the first multi-step task in a repository, add `TASKS.md` to its `.git/info/exclude` from the master checkout once; it covers every worktree, and Codex children cannot write the common `.git` from the sandbox.
   ...
   | low (低) | `low` | `low` |
4. Control: run this monitor loop while the child is active.
   1. Always keep exactly one background `orca orchestration check --run <run_id> --wait --types question,escalation,worker_done --timeout-ms 600000 --json` running (`run_in_background`) while a child is active. Never end a master turn without it.
   2. On `question`: answer with `orca orchestration reply --id <msg_id> --body "..."`; decisions outside the approved plan go to the user.
   3. On `escalation`: read it, decide, reply or correct with `orca terminal send`.
   4. On `worker_done`: acknowledge it once without waiting, `orca orchestration check --run <run_id> --ack <deliveryId> --json`, so the next task on the same Run does not receive a stale replay of it, then go to step 5 (final review).
   5. On timeout (no message in 10 minutes): run `worker-show --dispatch <id>` (state and `observation.agentWait`), `orca terminal read --terminal <handle> --screen | tail`, and check the `TASKS.md` mtime in the worktree. If the child is parked on a human-only prompt, stuck in a foreground command that never exits, or idle without a report, unblock it with `orca terminal send`. If it is progressing, do nothing.
   6. After every wake-up, re-arm the background `check` with `--ack <deliveryId>` of the batch just handled. Re-arming is mandatory in every branch, including timeout.

   Send spec additions and mid-run corrections with `orca terminal send --terminal <handle> --text "..." --enter --wait-submit 20`; do not recreate the worktree or restart the agent for a course correction. Ask the user only about decisions outside the approved plan. Never open a second agent session in the same task.
5. Final review. The `worker_done` body is the five-line report (result / changed files / verification: commands run + last lines of output / blocked on master / found); a `worker_done --outcome failed` is handled like a report with a blocker. Check the verification line first; if it has no evidence, send it back. Then check `git -C <worktree> status --short` and `git -C <worktree> diff` against the declared scope and spec. List only problems that would block the merge, each with file:line, why it is wrong, and reproduction steps that show it fails. Do not raise style preferences. For large diffs, delegate the diff read to a reviewer subagent to save master context. Send required fixes back to the child with `orca terminal send` and repeat; the dispatch is already settled, so the child's next `worker_done` is rejected but still arrives in the mailbox as "Rejected worker_done" with the full body - treat that body as the new report. After a rebase that moved the base, have the child rerun verification.
6. Commit and ship from the master session.
   ...
   ```
7. Clean up: `orca orchestration worker-release --dispatch <id>` when the child was started as a supervised worker. For a terminal bound with `--terminal`, this returns `retained` and closes nothing; it is still the correct call to run. `orca worktree rm --worktree name:<name>` is what actually closes the terminal.
