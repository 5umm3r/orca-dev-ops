...
- Direct edits in this checkout are limited to documentation (*.md, docs/, references/, .claude/) at any size, or Edit changes to other files of at most 2 files and 20 changed lines uncommitted in total; a PreToolUse guard blocks everything else. Documentation-only commits go directly on master and push; any commit with a non-documentation file goes on a task branch and fast-forwards.' ;;
child)
