...
3. When the five-line report arrives, check the verification line first; if it has no evidence (commands run + output), send it back. Then do the final review in the master: `git -C <worktree> status --short` and `git -C <worktree> diff` against the declared scope. List only problems that would block the merge, each with file:line, why it is wrong, and how to show it fails. For large diffs, delegate the diff read to a reviewer subagent to save master context. Send fixes back to the child if needed. After a rebase that moved the base, have the child rerun verification.
