...
child)
  ctx='[orca-workflow] This is a CHILD worktree session. Implement the plan the master session sent, run the required tests, and leave all changes uncommitted. Do not commit, push, merge, rebase, or create/remove worktrees; the master session does that. Ask the master through `orca orchestration ask` when the plan is unclear. When a step does not need input from the master, keep going. Stop and ask through `orca orchestration ask` only when you cannot continue without the master, or before anything destructive: deleting files outside this worktree, `git reset --hard` or `git clean -fd`, large dependency changes, and database or external-service operations. Permission prompts are off in this workflow, so this rule is the only check. Report in five lines (result / changed files / verification: commands run + last lines of output / blocked on master / found) through `orca orchestration send`.' ;;
*) exit 0 ;;
esac
